    ...
    public partial class Main : Form
    {
        ...
        private void BackstoreCsvExportMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (BackgroundWorker slave = new BackgroundWorker())
            {
                slave.DoWork += (s, args) =>
                {
                    string date = Microsoft.VisualBasic.Interaction.InputBox("Enter the date for CSV export (DD.MM.YYYY)", "Input data");
                    if (date == "")
                    {
                        return;
                    }
                    Telemetry.LogCompleteTelemetryData(connectionPara.TAG, Globals.Funkcje.BackstoreCsvExport, date);

                    Main.ChangeStatusBar("Checking CA.DE.BS.CSVExport.exe");
                    if(!File.Exists(@"\\" + connectionPara.TAG + @"\d$\TPDotnet\bin\CA.DE.BS.CSVExport.exe"))
                    {
                        CustomMsgBox.Show(CustomMsgBox.MsgType.Error, "CA.DE.BS.CSVExport.exe Error", @"CA.DE.BS.CSVExport.exe not found. Check if there is a backup exe at D:\TPDotnet\bin\CA.DE.BS.CSVExport_backup.exe and rename it if is there.");
                        return;
                    }

                    if (FileVersionInfo.GetVersionInfo(@"\\" + connectionPara.TAG + @"\d$\TPDotnet\bin\CA.DE.BS.CSVExport.exe").FileVersion == "6.6.6.6")
                    {
                        Logger.QuickLog(Globals.Funkcje.BackstoreCsvExport, date, connectionPara.TAG, "CriticalError", "CA.DE.BS.CSVExport.exe was already been replaced with modded one");
                        CustomMsgBox.Show(CustomMsgBox.MsgType.Error, "CA.DE.BS.CSVExport.exe Error", "CA.DE.BS.CSVExport.exe was already been replaced with modded one. Please restore original exe before trying to use this function to prevent any data loss or complications." + Environment.NewLine + @"Original exe should be in the same folder as modded one at D:\TPDotnet\bin\CA.DE.BS.CSVExport_backup.exe");
                        return;
                    }

                    Main.ChangeStatusBar("Installing modded CA.DE.BS.CSVExport.exe");
                    ...
                    {
                        Telemetry.LogMachineAction(connectionPara.TAG, Globals.Funkcje.Error, "Unable to rename CA.DE.BS.CSVExport.exe: " + moveExp.Message);
                        Logger.QuickLog(Globals.Funkcje.BackstoreCsvExport, date, connectionPara.TAG, "CriticalError", "Unable to rename CA.DE.BS.CSVExport.exe: " + Environment.NewLine + moveExp.ToString());
                        CustomMsgBox.Show(CustomMsgBox.MsgType.Error, "CA.DE.BS.CSVExport.exe Error", "Unable to rename CA.DE.BS.CSVExport.exe: " + moveExp.Message + " - Please check if CA.DE.BS.CSVExport.exe is ok");
                        return;
                    }
                    ...
                    {
                        Telemetry.LogMachineAction(connectionPara.TAG, Globals.Funkcje.Error, "Unable to copy modded CA.DE.BS.CSVExport.exe: " + moveExp.Message);
                        Logger.QuickLog(Globals.Funkcje.BackstoreCsvExport, date, connectionPara.TAG, "CriticalError", "Unable to rename CA.DE.BS.CSVExport.exe: " + Environment.NewLine + moveExp.ToString());
                        CustomMsgBox.Show(CustomMsgBox.MsgType.Error, "CA.DE.BS.CSVExport.exe Error", "Unable to copy modded CA.DE.BS.CSVExport.exe on to the host: " + copyExp.Message + Environment.NewLine + @"Please rename original exe from CA.DE.BS.CSVExport_backup.exe to CA.DE.BS.CSVExport.exe at D:\TPDotnet\bin");
                        return;
                    }

                    Main.ChangeStatusBar("Exporting CSV for " + date);
                    if (!CtrlFunctions.CsvExport(connectionPara, " " + date + " 49" + connectionPara.storeNr + @" D:\TPDotnet\Server\Reports\fiscal_files\", out string errorMsg))
                    {
                        Telemetry.LogMachineAction(connectionPara.TAG, Globals.Funkcje.Error, errorMsg);
                    ...
                    Main.ChangeStatusBar("Restoring original CA.DE.BS.CSVExport.exe");
                    ...
                    {
                        Telemetry.LogMachineAction(connectionPara.TAG, Globals.Funkcje.Error, "Unable to restore CA.DE.BS.CSVExport.exe: " + moveExp.Message);
                        Logger.QuickLog(Globals.Funkcje.BackstoreCsvExport, date, connectionPara.TAG, "CriticalError", "Unable to restore CA.DE.BS.CSVExport.exe: " + Environment.NewLine + moveExp.ToString());
                        CustomMsgBox.Show(CustomMsgBox.MsgType.Error, "CA.DE.BS.CSVExport.exe Error", "Unable to restore CA.DE.BS.CSVExport.exe: " + moveExp.Message + Environment.NewLine + @"Please rename original exe from CA.DE.BS.CSVExport_backup.exe to CA.DE.BS.CSVExport.exe at D:\TPDotnet\bin");
                        return;
                    }
                };
                slave.RunWorkerCompleted += (s, args) =>
                {
                    Main.ChangeStatusBar("Ready");
                };
                slave.RunWorkerAsync();
